...
impl SuperBlockState {
    ...
    pub fn build_superblock(
        ...
        let superblock = if let Some(sb) = sync_superblock {
            log::error!(
                "TENGO SUPERBLOQUE de tamaño {}",
                sb.signing_committee_length
            );
            log::error!("y el ars_previous es: [");
            for id in self.ars_previous_identities.ordered_identities.iter() {
                log::error!("{}", id);
            }
            log::error!("]");
            ...
            log::error!("NO TENGO SUPERBLOQUE");
            log::error!("y el ars_previous es: [");
            for id in self.ars_previous_identities.ordered_identities.iter() {
                log::error!("{}", id);
            }
        ...
        log::error!("Inserted a committee of {} [", self.signing_committee.len());
        for id in self.signing_committee.iter() {
            log::error!("{}", id);
        }
        log::error!("]");
